...
barplot(counts, main="classification based on emotion",
        xlab="emotion categories",ylab = "emotion_expressed") 
...
df <- data.frame(emotion_categories=c("unknown", "joy", "sadness","surprise","anger","fear","disgust"),
                 emotion_expressed=c(800,300,100,50,50,20,20))
head(df)
p<-ggplot(df, aes(x=emotion_categories, y=emotion_expressed, fill=emotion_categories)) +
